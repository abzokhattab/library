...
< % module_namespacing do
  -%>
RSpec.describe Api::V1::<%= controller_class_name.split("::").last %> Controller, < %= type_metatag(:routing) %> do
  describe "routing" do
...
< %
  unless options[:api] - %>
    it "routes to #edit" do
      expect(:get => "/<%= ns_table_name %>/1/edit"
    ).to route_to("<%= ns_table_name %>#edit", :id => "1")
  end
...
< % if Rails::VERSION::STRING > '4' - %>
    it "routes to #update via PATCH" do
      expect(:patch => "/<%= ns_table_name %>/1"
      ).to route_to("<%= ns_table_name %>#update", :id => "1")
    end
